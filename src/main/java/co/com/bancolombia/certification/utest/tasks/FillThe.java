...
public class FillThe implements Task {
    ...
    public FillThe(List<SignInInformation> signInInformationList) {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Click.on(CHK_STATE_INFORMED),
                Click.on(CHK_TERMS),
                Click.on(CHK_PRIVACY),
                Click.on(BTN_NEXT_STEP),
                WaitUntil.the(MSG_WELCOME, WebElementStateMatchers.isEnabled()).forNoMoreThan(120).seconds()

        );

    }

    public static FillThe credentials(List<SignInInformation> dataList) {
        return instrumented(FillThe.class, dataList);
    }
}
